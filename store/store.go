...
func (s *GarnetStore) Set(key, value string, ttlSeconds int) {
	...
	if ttlSeconds > 0 {
		...
		fmt.Println("Setting expiry for", key, "at", expires)
...
func (s *GarnetStore) Get(key string) (string, bool) {
	...
	if !e.expiresAt.IsZero() && time.Now().After(e.expiresAt) {
		return "", false // ✅ deny access if expired
	}
...
func (s *GarnetStore) StartTTLLoop(interval time.Duration) {
	go func() {
		for {
			time.Sleep(interval)
...
func (s *GarnetStore) cleanupExpiredKeys() {
	s.mu.Lock()
	defer s.mu.Unlock()
